...
echo '192.168.199.8 hadoop.example.com' >> /etc/hosts

# replace localhost in hadoop
cat /etc/ambari-agent/conf/ambari-agent.ini |sed 's/localhost/hadoop.example.com/g' > /etc/ambari-agent/conf/ambari-agent.ini.new
